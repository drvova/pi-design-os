...
export async function inspect(options = {}) {
  ...
  const order = payload.assets.order;

  return ok('inspect', {
    ...payload,
    assets: {
      ...payload.assets,
      order: order.slice(0, MAX_INLINE_ASSETS),
      orderShown: Math.min(order.length, MAX_INLINE_ASSETS),
      orderComplete: artefacts.report,
    },
    artefacts,
  });
}
...
export const COMMANDS = { directions, inspect };
